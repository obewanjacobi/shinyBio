...
p + stat_function(fun = fun.1) + xlim(0,200)
@
...
p <- ggplot(data = data.frame(x = 0), mapping = aes(x = x))
p + stat_function(fun = fun.1) + xlim(0,50)
@
...
xtable(tgriff, caption = 'Class That Used the App')
xtable(kopp, caption = 'Class Without App')
#these tables could be better, as Dr. White for tips
@
...
<<>>=
t.test(gpostscore,gprescore, paired = TRUE, data = tgriff, mu = 0)
@

\textbf{NOTE: This test is observing the class that used the app.}

<<>>=
t.test(kpostscore,kprescore, paired = TRUE, data = kopp, mu = 0)
@
...
We can definitely tell that the class that used the app improved more than the class that did not. This can be found simply by looking at the differences in the mean values for improvement in the grades, and the P values associated with them in the t-tests. The mean difference in the group that used the app is much greater than that of the group that didn't. On top of that, the P value of the group that used the app implies that there is indeed a correlation of great improvement when the app is used. On the other hand, the class that was without the app was very inconsistent. Their P value was higher than most statisticians would be comfortable saying anything about, and it stems from the fact that the students were very inconsistent in how well they improved going into the post test.
