...
A poisson distribution is also useful to our purposes because of the shape of the distribution. To show an example of this, view Figures 1 and 2. Here, the \(y\) axis represents the probability of \(x\) happening, and the \(x\) axis represents the number of occurences of the event that \(x\) defines. Notice that the values seem to peak somewhere around 1, because in these Figures, the expected value (or \(\lambda)\) for this kind of distribution) is 1. The graphs then have a tail (decrease towards 0) as the number of occurrences approaches infinity. This shape fits our situation of rabbit births quite well, as we know there will be an expected value of rabbit births, but we also know that the chance variation for rabbit births is likely to trail off to large numbers as well, but the likelyhood is much less for this to happen. 
...
@
\caption{This dot plot represents a standard Poisson distribution with an expected value of 1}
\end{figure}

\begin{figure}
<<echo=FALSE>>=
ggplot(transform(data.frame(x=c(0:10)), y=dpois(x, 1)), aes(x=x, y=y)) + 
    geom_bar(stat="identity")
@
\caption{This histogram represents a standard Poisson distribution with an expected value of 1}
\end{figure}
